            ...
            try {
                if (!booleanFlags.has("-s"))
                    try {
                        ...
                        notif = await dm.send(`You have been ${booleanFlags.has("-S") ? "soft" : ""}banned from **${message.guild?.name}** for \`${reason}\` for ${ms_1.default(time, { long: true })}${booleanFlags.has("-H")
