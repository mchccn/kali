...
`);
                    case "nuke":
                    case "n": {
                        const channel = yield message.channel.clone();
                        yield message.channel.delete();
                        const nuked = yield channel.send(`Channel <#${channel.name}> has been nuked.`);
                    ...
                    case "u": {
                        ...
                        const user = client.users.cache.get(args[index + 1].match(/(\d{18})/)[0]) ||
                            (yield client.users.fetch(args[index + 1]));
                        if (!user)
                            return message.channel.send(`Could not find that user.`);
                        const messages = new discord_js_1.Collection(Array.from((yield message.channel.messages.fetch({
                            limit: 100,
                        ...
                        if (booleanFlags.has("-s"))
                            return;
                        const done = yield message.channel.send(`**${count}** message${count !== 1 ? "s" : ""} w${count !== 1 ? "ere" : "as"} deleted.`);
            ...
            if (booleanFlags.has("-s"))
                return;
            const done = yield message.channel.send(`**${amount}** message${amount !== 1 ? "s" : ""} w${amount !== 1 ? "ere" : "as"} deleted.`);
            return done.delete({ timeout: 5000 });
        });
    },
};
