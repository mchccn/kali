                ...
                try {
                    if (!booleanFlags.has("-s"))
                        try {
                            ...
                            yield dm.send(`You have been unmuted in **${(_g = message.guild) === null || _g === void 0 ? void 0 : _g.name}** for \`${reason}\``);
                        }
                        catch (_h) {
                            yield message.channel.send(`Could not send DM to **${m.user.tag}**`);
                        }
                    if (!booleanFlags.has("-d") && m.roles.cache.has(role === null || role === void 0 ? void 0 : role.id))
                        yield m.roles.remove(role);
                    if (!booleanFlags.has("-d") &&
                        booleanFlags.has("-r") &&
                        m.roles.cache.has(react === null || react === void 0 ? void 0 : react.id))
                        yield m.roles.remove(react);
                }
                catch (e) {
                    console.log(e);
                    members.splice(i, 1);
