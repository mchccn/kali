...
`);
        ...
        await ping.edit(`Ping: ${client.ws.ping}ms | API Latency: ${ping.createdTimestamp - message.createdTimestamp}ms`);
        ...
        if (booleanFlags.has("-s") || booleanFlags.has("-D")) {
            ...
            for (let i = 1; i < 11; i++) {
                const msg = await message.channel.send(`Test ${i}: ${client.ws.ping}ms`);
                if (booleanFlags.has("-D")) {
                    const now = Date.now();
                    await msg.delete();
